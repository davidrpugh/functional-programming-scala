...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)((total, article) => if (article.text.split(" ").contains(lang)) total + 1 else total, _ + _)
  }
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    langs.map(lang => (lang, occurrencesOfLang(lang, rdd))).sortBy{ case (_, count) => -count }
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap[(String, WikipediaArticle)](article => langs.filter(lang => article.text.split(" ").contains(lang))
      .map(lang => (lang, article)))
      .map { case (lang, _) => (lang, 1) }
      .reduceByKey(_ + _)
      .sortBy { case (_, count) => -count }
      .collect()
      .toList
