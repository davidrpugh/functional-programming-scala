...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val fields = columnNames.map { name =>
      val dtype = if (name == columnNames.head) StringType else DoubleType
      StructField(name, dtype, nullable = false)
    }
    StructType(fields)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def isPrimaryNeed(name: String): Boolean = {
      name.startsWith("t01") || name.startsWith("t03") || name.startsWith("t11") || name.startsWith("t1801") || name.startsWith("t1803")
    }

    def isWork(name: String): Boolean = {
      name.startsWith("t05") || name.startsWith("t1805")
    }

    def isOther(name: String): Boolean = {
      !isPrimaryNeed(name) && !isWork(name) && !name.startsWith("t50")
    }

    columnNames.foldRight((List.empty[Column], List.empty[Column], List.empty[Column])) {
      case (name, (needs, work, other)) if isPrimaryNeed(name) => (new Column(name) :: needs, work, other)
      case (name, (needs, work, other)) if isWork(name) => (needs, new Column(name) :: work, other)
      case (name, (needs, work, other)) if isOther(name) => (needs, work, new Column(name) :: other)
      case (_, (needs, work, other)) => (needs, work, other)
    }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = {
      when('teage.between(15, 22), "young").when('teage.between(23, 55), "active").otherwise("elder").as("age")
    }
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy("working", "sex", "age")
      .agg(round(avg("primaryNeeds"), scale = 1), round(avg("work"), scale = 1), round(avg("other"), scale = 1))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .map { case ((working, sex, age), primaryNeeds, work, other) => TimeUsageRow(working, sex, age, round1(primaryNeeds), round1(work), round1(other)) }
      .orderBy('working, 'sex, 'age)
  }
