...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while (i < input.length){
      maximumAngle = max(maximumAngle, input(i) / i)
      output(i) = maximumAngle
      i += 1
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    var maximumAngle = startingAngle
    var i = from
    while (i < until) {
      maximumAngle = max(maximumAngle, input(i) / i)
      output(i) = maximumAngle
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit = tree match {
    ...
    case Node(left, right) =>
      parallel(downsweep(input, output, startingAngle, left), downsweep(input, output, max(left.maxPrevious, startingAngle), right))
  }

  /** Compute the line-of-sight in parallel. */
  def parLineOfSight(input: Array[Float], output: Array[Float], threshold: Int): Unit = {
    output(0) = 0
    val maxAngles = upsweep(input, 1, input.length, threshold)
    downsweep(input, output, 0, maxAngles)
  }
